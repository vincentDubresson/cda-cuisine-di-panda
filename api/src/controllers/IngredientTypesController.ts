...
export const post = async (req: Request, res: Response): Promise<void> => {
  const { type } = req.body;
  if (!type.trim()) {
    res.status(400).json({ error: "Le champ 'type d'ingrédient' est obligatoire." });
  } else {
    try {
...
export const put = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const updateIngredientType = await IngredientTypeRepository.updateIngredientType(id, type);
